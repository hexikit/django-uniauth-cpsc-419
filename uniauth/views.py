...
from uniauth.forms import (
    AddLinkedEmailForm,
    ChangePrimaryEmailForm,
    LinkedEmailActionForm,
    LoginForm,
    PasswordChangeForm,
    PasswordResetForm,
    SetPasswordForm,
...
from uniauth.tokens import get_jwt_tokens_for_user, token_generator
...
def _get_redirect_url_with_tokens(url, access, refresh, userid, username):
    query_string = f'?access={access}&refresh={refresh}&userid={userid}&username={username}'
...
def _login_success(request, user, next_url, drop_params=[]):
    ...
    if is_tmp_user(user):
        ...
        return HttpResponseRedirect(
            reverse("uniauth:link-to-profile") + "?" + urlencode(query_params)
        )
...
def login(request):
    ...
    if not display_standard and not display_cas:
        err_msg = "At least one of '%s' and '%s' must be True." % (
            ...
            "UNIAUTH_LOGIN_DISPLAY_CAS",
        )
        raise ImproperlyConfigured(err_msg)
    if display_cas and num_institutions == 0:
        err_msg = (
            "'%s' is True, but there are no Institutions in the "
...
def signup(request):
    ...
    if (
        ...
        and not is_tmp_user(request.user)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return render(
                request,
                "uniauth/verification-waiting.html",
...
def settings(request):
    ...
    if is_unlinked_account(request.user):
        ...
        return HttpResponseRedirect(
            reverse("uniauth:link-to-profile") + "?" + params
        )
    ...
    if request.method == "POST":
            ...
            if action_email_form.is_valid():
                ...
                if delete_pk:
                    try:
                        ...
                        if (
                            email.profile.user == request.user
                            and email.address != request.user.email
                        ):
                            email.delete()
        ...
        # validate and set user's primary email address
            ...
            change_email_form = ChangePrimaryEmailForm(
...
def _add_institution_account(profile, slug, cas_id):
    ...
    InstitutionAccount.objects.create(
        profile=profile, institution=institution, cas_id=cas_id
    )
...
def verify_token(request, pk_base64, token):
    ...
    if (
        email is not None
        and is_tmp_user(email.profile.user)
        and get_user_model().objects.filter(email=email.address).exists()
    ):
        email = None
    ...
    if email is not None and token_generator.check_token(email, token):
        ...
        if not get_setting("UNIAUTH_ALLOW_SHARED_EMAILS"):
            LinkedEmail.objects.filter(
                address=email.address, is_verified=False
            ).delete()
...
class PasswordResetVerify(PasswordResetConfirmView):
    ...
    def form_valid(self, form):
        ...
        """
        query_params = self.request.session.pop(
...
class PasswordResetVerifyDone(PasswordResetCompleteView):
    ...
    def get_context_data(self, **kwargs):
        ...
        """
        context = super(PasswordResetVerifyDone, self).get_context_data(
            **kwargs
        )
